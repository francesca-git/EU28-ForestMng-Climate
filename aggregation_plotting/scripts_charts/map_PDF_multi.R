...
plot.map <- function(folder_slost, file_slost, case_subcase, plots_path, id, energy_exports, map)  { 
  ...
  # Zoom ====
    ...
    EU_map <- st_read(dsn = "./aggregation_plotting/Map_EU_dissolved", layer = "dissolved_EU")
    
    EU_map_shp <- readOGR(dsn = "./aggregation_plotting/Map_EU_dissolved", layer = "dissolved_EU")
