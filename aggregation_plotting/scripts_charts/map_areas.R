...
# General task: plot the maps of the global land use
# Date: September 2020
...
plot.map.areas <- function(folder_areas, mainfolder)  { 
    ...
    if (length(year) > 1) {
      ...
      png(file = paste0(mainfolder, "/areas/NotRel-Wet/", map, "_", climate, "_", region, "_NotRelWet.png"), width = 9, height = 4.5, res = 1000, units = "in")
    
      } else{
        
      #pdf(file = paste0("./NotRel-Wet/", map, "_", climate, "_", year[1], "_", region,"_", test, ".pdf"), width = 14, height = 14)
      png(file = paste0(mainfolder, "/areas/NotRel-Wet/", map, "-", id,"_", climate, "_", year[1], "_", region, "_NotRelWet.png"), width = 14, height = 14, res = 1000, units = "in")
