...
################## EU INTERNAL #######################
  ...
  results_sum <- results_temp %>%
    ...
    pivot_longer(cols = 3:(length(results_sum)), names_to = "Category", values_to = "PDFx100")
    
  
  write.csv(results_oneyear, paste0(save_csv_path, "EUForest_", year, case, ".csv"), row.names = FALSE)
  write.csv(results_points, paste0(save_csv_path, "EUForest_", year, case, "_points.csv"), row.names = FALSE)
