...
# The functions differ according to whether they include energy plantations/permanent crops and/or exports in the impacts.  
    # 1) create.csv.EU.ex - Energy plantations and EU energy crops included for the EU Footprint, but for EU internal. Exports included. 
...
create.csv.EU.ex <- function(rdata_path, csv_path, case_subcase) {
  ...
  ################## EU FOOTPRINT #######################
    ...
    ######## SUMS OF GROUPED LAND USE CLASSES ########  
        ...
        # group the land use classes in: EU28_Energy_crops, EU_Forest_net, Import_Energy_plantations, Import_Forest
          ...
          write.csv(results_oneyear, paste0(csv_path, "EUFootprint_", year, case_subcase, "_EP.csv"), row.names = FALSE)
          # this csv has the following columns: 
          # Group (climatic and forest use scenario), Scenario (forest management scenario), Category (land use category), PDFx100 (impact)
    ...
    ######## TOTAL SUMS ########
      ...
      # prepare and save the file with the total sums for the given year 
         ...
         write.csv(sums_oneyear, paste0(csv_path, "EUFootprint_", year, case_subcase, "_top_EP.csv"), row.names = FALSE) 
                # this csv has the following columns: 
                # Group (climatic and forest use scenario), Scenario (forest management scenario), PDFx100 (impact), lower95 (lower CI interval), upper95 (upper CI interval)
                # If columns lower95 and upper95 are all 0 it means that CI have not been calculated
  ...
  ################## EU INTERNAL #######################
    ...
    ######## SUMS OF GROUPED LAND USE CLASSES ########  
      ...
      # group the land use classes in: Clear_cut, Retention, Selection, Other_management
        ...
        results_temp <- results_median[[year]] %>%
          ...
          write.csv(results_oneyear, paste0(csv_path, "EUForest_", year, case_subcase, "_noEPex.csv"), row.names = FALSE)
          # this csv has the following columns: 
          # Group (climatic and forest use scenario), Scenario (forest management scenario), Category (land use category), PDFx100 (impact)
    ...
    ######## TOTAL SUMS ########
      ...
      # prepare and save the file with the total sums for the given year 
        ...
        write.csv(sums_oneyear, paste0(csv_path, "EUForest_", year, case_subcase, "_top_noEPex.csv"), row.names = FALSE)
          # this csv has the following columns: 
          # Group (climatic and forest use scenario), Scenario (forest management scenario), PDFx100 (impact), lower95 (lower CI interval), upper95 (upper CI interval)
          # If columns lower95 and upper95 are all 0 it means that CI have not been calculated
...
##################################################### TIME SERIES OF IMPACTS ##################################################### 
  ...
  ################## EU FOOTPRINT #######################
    ...
    df <- sums[[1]][1,] %>%
      ...
      write.csv(df, paste0(csv_path, "EUFootprint_time-series", case_subcase, "_EP.csv"), row.names = FALSE) 
      # this csv has the following columns: 
  ...
  ################## EU INTERNAL #######################
    ...
    df <- sums[[1]][1,] %>%
      ...
      write.csv(df, paste0(csv_path, "EUForest_time-series", case_subcase, "_noEPex.csv"), row.names = FALSE) 
      # this csv has the following columns: 
      # Group (climatic and forest use scenario), PDFx100 (impact), lower95 (lower CI interval), upper95 (upper CI interval), Scenario (forest management scenario), Year
...
create.csv.EU.EPnoex <- function(rdata_path, csv_path, case_subcase) {
    ...
    ######## SUMS OF GROUPED LAND USE CLASSES ########  
      ...
      # group the land use classes in: Clear_cut, Retention, Selection, Other_management
        ...
        results_temp <- results_median[[year]] %>%
          ...
          write.csv(results_oneyear, paste0(csv_path, "EUForest_", year, case_subcase, "_EPnoex.csv"), row.names = FALSE)
          # this csv has the following columns: 
          # Group (climatic and forest use scenario), Scenario (forest management scenario), Category (land use category), PDFx100 (impact)
    ...
    ######## TOTAL SUMS ########
      ...
      # prepare and save the file with the total sums for the given year 
        ...
        sums_oneyear = results[[year]] %>% 
          ...
          write.csv(sums_oneyear, paste0(csv_path, "EUForest_", year, case_subcase, "_top_EPnoex.csv"), row.names = FALSE) 
          # this csv has the following columns: 
          # Group (climatic and forest use scenario), Scenario (forest management scenario), PDFx100 (impact), lower95 (lower CI interval), upper95 (upper CI interval)
          # If columns lower95 and upper95 are all 0 it means that CI have not been calculated
...
create.csv.EU.noEPnoex <- function(rdata_path, csv_path, case_subcase) {
  ...
  ################## EU FOOTPRINT #######################
    ...
    ######## SUMS OF GROUPED LAND USE CLASSES ########  
        ...
        # group the land use classes in: EU28_Energy_crops, EU_Forest_net, Import_Energy_plantations, Import_Forest
          ...
          write.csv(results_oneyear, paste0(csv_path, "EUFootprint_", year, case_subcase, "_noEP.csv"), row.names = FALSE)
          # this csv has the following columns: 
          # Group (climatic and forest use scenario), Scenario (forest management scenario), Category (land use category), PDFx100 (impact)
    ...
    ######## TOTAL SUMS ########
      ...
      # prepare and save the file with the total sums for the given year 
          ...
          write.csv(sums_oneyear, paste0(csv_path, "EUFootprint_", year, case_subcase, "_top_noEP.csv"), row.names = FALSE) 
          # this csv has the following columns: 
          # Group (climatic and forest use scenario), Scenario (forest management scenario), PDFx100 (impact), lower95 (lower CI interval), upper95 (upper CI interval)
          # If columns lower95 and upper95 are all 0 it means that CI have not been calculated
    ...
    ######## SUMS OF GROUPED LAND USE CLASSES ########  
      ...
      # group the land use classes in: Clear_cut, Retention, Selection, Other_management
        ...
        results_temp <- results_median[[year]] %>%
          ...
          write.csv(results_oneyear, paste0(csv_path, "EUForest_", year, case_subcase, "_noEPnoex.csv"), row.names = FALSE)
          # this csv has the following columns: 
          # Group (climatic and forest use scenario), Scenario (forest management scenario), Category (land use category), PDFx100 (impact)
    ...
    ######## TOTAL SUMS ########
      ...
      # prepare and save the file with the total sums for the given year 
        ...
        sums_oneyear = results[[year]] %>% 
          ...
          write.csv(sums_oneyear, paste0(csv_path, "EUForest_", year, case_subcase, "_top_noEPnoex.csv"), row.names = FALSE) 
          # this csv has the following columns: 
          # Group (climatic and forest use scenario), Scenario (forest management scenario), PDFx100 (impact), lower95 (lower CI interval), upper95 (upper CI interval)
          # If columns lower95 and upper95 are all 0 it means that CI have not been calculated
