      ...
      # multiply Slost by the allocation factor and the weights
       ...
       #### RESULTS PER TAXA ####
        ...
        Slost_taxa_matrix <- list()   # list to be filled out with the results per taxa (each element of the list will be a matrix with the impacts per ecoregion and land use category)
          ...
          ptm <- proc.time()
        if (BS == TRUE) {
          Slost_pertaxon <- apply(Slost_lf, c(1,2,3), mean, na.rm = TRUE)                   # compute the mean of the Slost computed using the bootstrapped values of rr adn z
       ...
       #### AGGREGATED RESULTS ####
        ...
        if (uncertainties == FALSE) {
          Slost_aggr_2.5 <- 0
          Slost_aggr_97.5 <- 0
        } else if(uncertainties == TRUE & BS == FALSE) {
