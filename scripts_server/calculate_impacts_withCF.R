...
setwd("/home/frrosa/R/Forest_management/") 
...
scenarios <- read.csv("./comparison_LCImpact/Scenarios.csv")
scenarios = unlist(unique(scenarios))
nscenarios <- length(scenarios)

Ecoregions <- read.csv("./comparison_LCImpact/Ecoregion_codes.csv")
...
temp = list.files(path = "./comparison_LCImpact/areas/notimber/mg/aggregated/", pattern="*.csv", full.names = TRUE)
myfiles = lapply(temp, read.csv)
fr_temp = list.files(path = "./comparison_LCImpact/areas/notimber/mg/fraction/", pattern="*.csv", full.names = TRUE)
...
# Packages need to be passed on to each core
  ...
  results = foreach(t = 1:length(tstep), .packages=c('dplyr', 'tidyr', 'abind', 'tidyverse')) %dopar% {
    ...
    write.csv(Slost_fin_disaggr, paste0("./comparison_LCImpact/results/Slost_mg_", tstep[t], "_", end_name, ".csv"), row.names = FALSE)
