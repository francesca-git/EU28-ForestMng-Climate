...
source("./parameters_calculation.R")
  ...
  calculate.slost <- function(Total_RemainingNatural_areas, Land_use_areas, CI, vulnerability, cutoff) { 
    ...
    print(paste0("CI : ", CI))
    if (CI == FALSE) {uncertainties = FALSE # it can be "lognormal" or "Weibull" or FALSE - default: FALSE -> to calculate confidence intervals
                    simulations = 2}else if(CI == TRUE){uncertainties = TRUE
    ...
    # The parameters are calculated using the functions in the file parameters_calculation.R
      ...
      load("rr_ecoregion.Rdata")
      ratio_eco = rr_ecoregion # calculate.RR(uncertainties = uncertainties, distribution = distribution, cutoff = cutoff)        # four dimension array containing the response ratio per ecoregion (1st dimensio), land use category (2nd dimension) and taxa (3rd dimension).
      ...
      if (dim(ratio_eco)[4] != simulations) {stop("Problem with the dimension of the array containing the response ratios (function: calculate.slost)")}
      ...
      zvalues = prepare.zvalues(uncertainties)
    ...
    ############################# CALCULATION OF THE IMPACTS ############################# 
      ...
      # no aggregation, as in eq. 11.3 of LCImpact method for land stress (Chaudhary, 2016), but multiplied also by the vulnerability scores)
        ...
        system.time(  
        Slost_h_a_suit <- specieslost_sim(Sorg_VS, Total_RemainingNatural_areas$A_org, Total_RemainingNatural_areas$A_new, Land_use_areas, zvalues, ratio_eco, n = simulations)
        )
      ...
      # Results per taxa 
        ...
        Slost_df <- apply(Slost_lf, c(1,2,3), mean, na.rm = TRUE)                           # compute the median of the impacts
