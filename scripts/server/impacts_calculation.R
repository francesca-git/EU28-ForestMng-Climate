...
############################# DEFINE THE SCENARIOS/CONDITIONS UNDER WHICH PERFORM THE ANALYSIS ############################# 
  ...
  # Calculation of the impacts
    ...
    Full = "OFF"             # "ON" or "OFF". "ON": the results are dis-aggregated according to the land use classes provided by Fulvio; "OFF": the impacts are aggregated according to Chaudhary 2015 (broad land use classes) and Chaudhary 2016 (forest use intensities) papers 
  ...
  # Calculation of the response ratio
    distribution = "Weibull" # distribution: "lognormal" or "Weibull" or "OFF" - default: "OFF" -> to calculate confidence intervals
...
############################# CALCULATE THE INPUT PARAMETERS ############################# 
  ...
  # The parameters are calculated using the functions in the file parameters_calculation.R
    ...
    ratio_eco = calculate.RR(distribution, cutoff = cutoff)       # four dimension array containing the response ratio per ecoregion (1st dimensio), land use category (2nd dimension) and taxa (3rd dimension).
    ...
    # "Annual"               "Permanent"            "Pasture"              "Urban"                "PlantationNonTimber"  "ClearCut"             "Retention"            "PlantationFuel"      
    # "SlashAndBurn"         "SelectionSystem"      "SelectiveLogging"     "Agroforestry"         "Plantation"           "ReducedImpactLogging" "Afforested"           "ForExtensive"        
    # "ForIntensive"  
    
    # weight_tx = prepare.Sorg.VS.weighting(vulnerability = vulnerability)[["Weights"]]    #
...
############################# LOAD THE AREAS ############################# 
    ...
    # create vectors which contain the columns to select and replicate from ratio_eco to make them suitable for the matching with the areas (functions from the file support.R)
    
      lu_to_assess = arrange_columns() # this is a character vector

      ratio_eco <- ratio_eco[,lu_to_assess,,]
      lf_use_type = c(dimnames(ratio_eco)[[2]])
      nlfuse = dim(ratio_eco)[2]
      
      rm(lu_to_assess, temp, fr_temp)
...
############################# CALCULATE THE IMPACTS ############################# 
  ...
  # Parallel for-loop
    ...
    # Parallel for-loop
    # Packages need to be passed on to each core
    results = foreach(t = 1:ntstep, .packages=c('dplyr', 'tidyr', 'abind', 'tidyverse')) %dopar% {
      # t = 11
      #AREAS 
      ...
      names(Slost_fin) = c("Scenario", "Eco_code", "Year", paste(lf_use_type, "median", sep = "_"), paste(lf_use_type, "lower95", sep = "_"), paste(lf_use_type, "upper95", sep = "_"))
      ...
      for (sc in scenario_in) {
        ...
        ##### COMPUTE THE TOTAL IMPACTS AND THE IMPACTS PER TAXONOMIC GROUP #####
          ...
          Slost <- calculate.impacts(Total_RemainingNatural_areas = Areas_org_new, Land_use_areas = Areas_lu, CI = FALSE, ratio_eco = ratio_eco, vulnerability = vulnerability)
          # Arguments:
          ...
          # ecoregions and are sorted in alphabetical order according to the ecoregion codes (AA0101, AA0102, etc.)
          # 3) CI can be TRUE or FALSE. TRUE = the CI will be calculated, FALSE = the CI will not be calculated
          # 4) ratio_eco: 4dimension array with the response ratios
          ...
          if (CI == TRUE) {
            Slost_CI <- calculate.impacts(Total_RemainingNatural_areas = Areas_org_new, Land_use_areas = Areas_lu, CI = TRUE, ratio_eco = ratio_eco, vulnerability = vulnerability)
        ...
        # if (distribution == "OFF") {
        #   
        #   Slost_fin_plants <- Slost_fin_plants %>% mutate(select(contains("uppper95")) = 0) %>%
        #                                             mutate(select(contains("lower95")) = 0) 
        #   Slost_fin_birds = Slost_taxa_matrix[["Birds"]]
        #   Slost_fin_mammals = Slost_taxa_matrix[["Mammals"]]
        #   
        # }
...
############################# SAVE THE RESULTS ############################# 
      ...
      # Slost_fin <- Slost_fin %>% separate(col = Scenario, into = c("Climate", "Forest_use", "Management"), 
      #                                     sep = "_", remove = TRUE)
      # 
      if (Full == "OFF") {
        ...
        if (Approach == "AV" && vulnerability == "OFF") {
          write.csv(Slost_fin, paste0("./species-lost/av/Slost_av_", tstep[t], "_noV_disaggr.csv"), row.names = FALSE)
        } else if (Approach == "AV" && vulnerability == "ON") {
          write.csv(Slost_fin, paste0("./species-lost/av/Slost_av_", tstep[t], "_disaggr.csv"), row.names = FALSE)
        } else if(Approach == "MG" && vulnerability == "OFF") {
          write.csv(Slost_fin, paste0("./species-lost/Slost_mg_", tstep[t], "_noV_disaggr.csv"), row.names = FALSE)
        } else if (Approach == "MG" && vulnerability == "ON") {
          write.csv(Slost_fin_disaggr, paste0("./species-lost/Slost_mg_", tstep[t], "_disaggr.csv"), row.names = FALSE)
          write.csv(Slost_fin_disaggr_plants, paste0("./species-lost/Slost_mg_", tstep[t], "_disaggr_plants.csv"), row.names = FALSE)
          write.csv(Slost_fin_disaggr_birds, paste0("./species-lost/Slost_mg_", tstep[t], "_disaggr_birds.csv"), row.names = FALSE)
          write.csv(Slost_fin_disaggr_mammals, paste0("./species-lost/Slost_mg_", tstep[t], "_disaggr_mammals.csv"), row.names = FALSE)
        }
