...
source("./parameters_calculation.R")
  ...
  calculate.slost <- function(Total_RemainingNatural_areas, Land_use_areas, CI, vulnerability, cutoff) { 
    ...
    print(paste0("CI : ", CI))
    if (CI == FALSE) {distribution = "OFF" # it can be "lognormal" or "Weibull" or "OFF" - default: "OFF" -> to calculate confidence intervals
                    simulations = 2}else if(CI == TRUE){distribution = "Weibull"
                                                          simulations = 1000}
    ...
    # The parameters are calculated using the functions in the file parameters_calculation.R
      ...
      ratio_eco = calculate.RR(distribution = distribution, cutoff = cutoff)        # four dimension array containing the response ratio per ecoregion (1st dimensio), land use category (2nd dimension) and taxa (3rd dimension).
