...
# ====
          ...
          data$Scenario <- factor(data$Scenario, levels = c("2020-RCP6.5/RCP2.6", "2100-RCP6.5", "2100-RCP2.6"), labels = c("2020 - RCP6.5 / RCP2.6", "2100 - RCP6.5", "2100 - RCP2.6"))
            ...
            print("EUFootprint")
              if(graph == "B-25-50") {
                ...
                print("B-25")
                data <- data %>% filter(Level == "25%" | Level == "Baseline" & Scenario == "Close-to-nature") %>%
                                    unite("Scenario", c(Scenario, Level), sep = "-", remove = TRUE) %>%
                                      mutate(Scenario = str_replace(Scenario, "Close-to-nature-Baseline", "Baseline"))
                data$Group = factor(data$Group, levels = c("RCP6.5", "RCP2.6"), labels = c("RCP6.5", "RCP2.6"))
                data$Scenario = factor(data$Scenario, levels = c("Baseline", "Close-to-nature-25%", "Set-aside-25%"), labels = c("Baseline (noAFM)", "Close-to-nature 25%", "Set-aside 25%"))
  ...
  if (length(year) > 1) {
    ...
    png(file = paste0(plots_path, map, "-", id,"_", climate, "_", year[1], "_", region, file_label, "_",energy_exports, "_25.png"),  width = 10, height = 5, res = 600, units = "in") # 20, 10
  ...
  # max for EUFootprint: 0.007033453
     ...
     if(id == "EUFootprint")  {# & file_label != "_cutoff_mammals") {
       ...
       br <- c(0, 0.005, 0.010, 0.015, 0.020)
     }
    # 
