...
plot.map <- function(folder_slost, file_slost, case_subcase, plots_path, id)  { 
        ...
        mutate_if(is.numeric, ~.*100) %>%
          mutate(Global = rowSums(select(., contains("median"))),
                  EUFootprint = rowSums(select(., contains("median") & ((contains("EP") & contains("EU"))|(starts_with("For") & contains("EU"))|(contains("EP") & contains("im"))|(starts_with("For_") & contains("im"))))),
                    EUForest = rowSums(select(., contains("median") & (((starts_with("For_") | starts_with("EP") ) & (contains("EU"))) | (contains("ForOther") & contains("EU")))))) %>%
  ...
  data_bu <- data
  data <- data_bu %>% mutate(Values = log10(Values)) 
  ...
  if (length(year) > 1) {
    ...
    png(file = paste0(plots_path, map, "_", climate, "_", region, case_subcase, "_EP_noex_setac_blues_log.png"), width = 6, height = 10, res = 600, units = "in")
    } else{
      
    #pdf(file = paste0("./plotting/no_cutoff/", map, "-", id,"_", climate, "_", year[1], "_", region, "_", test,"lr.pdf"), width = 8, height = 4)
    png(file = paste0(plots_path, map, "-", id,"_", climate, "_", year[1], "_", region, case_subcase, "_EP_noex_setac_log.png"),  width = 6, height = 10, res = 600, units = "in")
  ...
  if (region != "global") {
    figure <- figure + facet_grid(rows = vars(Scenario)) # figure <- figure + facet_grid(Group ~ Scenario) 
