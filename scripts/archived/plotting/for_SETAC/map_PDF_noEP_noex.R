...
plot.map <- function(folder_slost, file_slost, case_subcase, plots_path, id)  { 
      ...
      data <- data %>%  
        mutate_if(is.numeric, ~.*100) %>%
          mutate(Global = rowSums(select(., contains("median"))),
                  EUFootprint = rowSums(select(., contains("median") & ((starts_with("For") & contains("EU"))|(starts_with("For_") & contains("im"))))),
                    EUForest = rowSums(select(., contains("median") & ((starts_with("For_") & contains("EU")) | (contains("ForOther") & contains("EU")))))) %>%
  ...
  if (length(year) > 1) {
    ...
    png(file = paste0(plots_path, map, "_", climate, "_", region, case_subcase, "_noEP_noex.png"), width = 6, height = 6, res = 600, units = "in")
    } else{
      
    #pdf(file = paste0("./plotting/no_cutoff/", map, "-", id,"_", climate, "_", year[1], "_", region, "_", test,"lr.pdf"), width = 8, height = 4)
    png(file = paste0(plots_path, map, "-", id,"_", climate, "_", year[1], "_", region, case_subcase, "_noEP_noex.png"),  width = 6, height = 6, res = 600, units = "in")
