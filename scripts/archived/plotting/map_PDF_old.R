...
# ====
  ...
  # Zoom ====
    ...
    ### Code needed to plot the EU map after having re-allocated the impacts to the EU region using the Globiom share of each ecoregion 
      #
      # Globiom_eco_org <- read.csv("./input_data/grouped_land_use_files/GLOBIOM_Ecoregion.csv", header = TRUE)
      ...
      # 
      # data <- join_regions(data, Globiom_eco_org) %>% # multiplies all the values for the relative share at Globiom region
      #         filter(Globiom_Reg == region)
      # 
      # df <- left_join(shp, data)
      # 
      # EU_map <- st_read(dsn = "./aggregation_plotting/maps_shapefiles/Map_EU_dissolved", layer = "dissolved_EU")
      # 
      # EU_map_shp <- readOGR(dsn = "./aggregation_plotting/maps_shapefiles/Map_EU_dissolved", layer = "dissolved_EU")
      # 
      # ggplot() +
      #       geom_sf(data = EU_map, fill = "transparent", colour = "black") 
      # 
      # sf_use_s2(FALSE)
      # EU_border <- st_intersection(df, EU_map)
      # sf_use_s2(TRUE)
      # 
      # df <- EU_border
  ...
  if (length(year) > 1) {
    ...
    png(file = paste0(plots_path, map, "-", id,"_", climate, "_", year[1], "_", region, file_label, "_",energy_exports, ".png"),  width = 14, height = 24, res = 600, units = "in")
