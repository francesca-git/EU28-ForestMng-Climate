...
#############################################################################################################################################################################
  ...
  create.csv.global <- function(CI, file_rdata_path, csv_path, file_label, year) {
            ...
            # without this condition the test here below will always fail when the option vulnerability is FALSE.
                    ...
                    test_check = results[[year]][nrow(results_temp),]
                        ...
                        (abs(test$Forest - (test_check$For_ClearCut_EU_median +  test_check$For_ClearCut_im_median + test_check$For_ClearCut_ex_median +
                          ...
                          test_check$For_SelectionSystem_im_median + test_check$For_SelectionSystem_ex_median + test_check$For_Selective_im_median + test_check$For_ReducedImpactLogging_im_median + test_check$For_TimberPlant_EU_median + test_check$For_TimberPlant_ex_median)) > 1e-15) |
