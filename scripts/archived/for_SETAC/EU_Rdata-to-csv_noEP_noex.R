...
create.csv.EU <- function(CI, rdata_path, csv_path, case_subcase) {
  ...
  ################## EU FOOTPRINT #######################
    ...
    ######## SUMS OF GROUPED LAND USE CLASSES ########  
        ...
        # group the land use classes in: EU28_Energy_crops, EU_Forest_net, Import_Energy_plantations, Import_Forest
          ...
          write.csv(results_oneyear, paste0(csv_path, "EUFootprint_", year, case_subcase, "_noEP_noex.csv"), row.names = FALSE)
          # this csv has the following columns: 
          # Group (climatic and forest use scenario), Scenario (forest management scenario), Category (land use category), PDFx100 (impact)
    ...
    ######## TOTAL SUMS ########
      ...
      # prepare and save the file with the total sums for the given year 
        ...
        if (CI == TRUE) { write.csv(sums_oneyear, paste0(csv_path, "EUFootprint_", year, case_subcase, "_top_CI_noEP_noex.csv"), row.names = FALSE) 
                } else if (CI == FALSE) { write.csv(sums_oneyear, paste0(csv_path, "EUFootprint_", year, case_subcase, "_top_noEP_noex.csv"), row.names = FALSE) }
                # this csv has the following columns: 
                # Group (climatic and forest use scenario), Scenario (forest management scenario), PDFx100 (impact), lower95 (lower CI interval), upper95 (upper CI interval)
                # If CI == FALSE, the columns lower95 and upper95 are all 0
    ...
    ######## SUMS OF GROUPED LAND USE CLASSES ########  
      ...
      # group the land use classes in: Clear_cut, Retention, Selection, Other_management
        ...
        results_temp <- results_median[[year]] %>%
          ...
          write.csv(results_oneyear, paste0(csv_path, "EUForest_", year, case_subcase, "_noEP_noex.csv"), row.names = FALSE)
          # this csv has the following columns: 
          # Group (climatic and forest use scenario), Scenario (forest management scenario), Category (land use category), PDFx100 (impact)
    ...
    ######## TOTAL SUMS ########
      ...
      # prepare and save the file with the total sums for the given year 
        ...
        sums_oneyear = results[[year]] %>% 
          ...
          if (CI == TRUE) { write.csv(sums_oneyear, paste0(csv_path, "EUForest_", year, case_subcase, "_top_CI_noEP_noex.csv"), row.names = FALSE) 
            } else if (CI == FALSE) { write.csv(sums_oneyear, paste0(csv_path, "EUForest_", year, case_subcase, "_top_noEP_noex.csv"), row.names = FALSE) }
          # this csv has the following columns: 
          # Group (climatic and forest use scenario), Scenario (forest management scenario), PDFx100 (impact), lower95 (lower CI interval), upper95 (upper CI interval)
          # If CI == FALSE, the columns lower95 and upper95 are all 0
...
##################################################### TIME SERIES OF IMPACTS ##################################################### 
  ...
  ################## EU FOOTPRINT #######################
    ...
    df <- sums[[1]][1,] %>%
      ...
      for(i in 1:length(tstep)) {
        ...
        results[[i]]$Year <- toString(tstep[i])
         ...
         if (abs(test$PDFx100 - (
              ...
              test_check$ForOther_Intensive_EU_median + 
      ...
      df <- df %>% 
       ...
       if (CI == TRUE) { write.csv(df, paste0(csv_path, "EUFootprint_time-series", case_subcase, "_CI_noEP_noex.csv"), row.names = FALSE)
                } else if (CI == FALSE) { write.csv(df, paste0(csv_path, "EUFootprint_time-series", case_subcase, "_noEP_noex.csv"), row.names = FALSE) }
      # this csv has the following columns: 
  ...
  ################## EU INTERNAL #######################
    ...
    df <- sums[[1]][1,] %>%
      rename(PDFx100 = Sum_median, upper95 = Sum_upper95, lower95 = Sum_lower95) %>%
        ...
        for(i in 1:length(tstep)) {
          ...
          temp <- results[[i]] %>% unite("Group", Group:Forest_use, remove = TRUE) %>%
            rename(Scenario = Management) %>% 
              transmute(Group = Group, Scenario = Scenario, Year = Year,
                    PDFx100 = rowSums(dplyr::select(., contains("median") &  ((contains("ClearCut") & contains("EU"))|
                                                                                (contains("Retention") & contains("EU"))|
                                                                                (contains("Selection") & contains("EU"))|
                                                                                (contains("Timber") & contains("EU"))|
                                                                                (contains("ForOther") & contains("EU"))))),
                    lower95 = rowSums(dplyr::select(., contains("lower") &  ((contains("ClearCut") & contains("EU"))|
                                                                                (contains("Retention") & contains("EU"))|
                                                                                (contains("Selection") & contains("EU"))|
                                                                                (contains("Timber") & contains("EU"))|
                                                                                (contains("ForOther") & contains("EU"))))),
                    upper95 = rowSums(dplyr::select(., contains("upper") &  ((contains("ClearCut") & contains("EU"))|
                                                                                (contains("Retention") & contains("EU"))|
                                                                                (contains("Selection") & contains("EU"))|
                                                                                (contains("Timber") & contains("EU"))|
                                                                                (contains("ForOther") & contains("EU"))))))
      ...
      if (CI == TRUE) { write.csv(df, paste0(csv_path, "EUForest_time-series", case_subcase, "_CI_noEP_noex.csv"), row.names = FALSE) 
                } else if (CI == FALSE) { write.csv(df, paste0(csv_path, "EUForest_time-series", case_subcase, "_noEP_noex.csv"), row.names = FALSE) }
      # this csv has the following columns: 
      # Group (climatic and forest use scenario), PDFx100 (impact), lower95 (lower CI interval), upper95 (upper CI interval), Scenario (forest management scenario), Year
