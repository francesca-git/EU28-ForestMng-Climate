...
#shp_original <- st_polygon(dsn = "./scripts/plotting/maps_shapefiles/WWF_Ecoregions", layer = "wwf_terr_ecos")

  a = data.frame(NAME = "A")
  st_geometry(a) = shp$geometry %>% st_sfc()
  ...
  if (length(year) > 1) {
    ...
    #png(file = paste0(plots_path, map, "-", id,"_", climate, "_", year[1], "_", region, file_label, "_",energy_exports, "_diff.png"),  width = 5, height = 9, res = 600, units = "in") # 10, 18
      png(file = paste0(plots_path, map, "-", id,"_", climate, "_", year[1], "_", region, file_label, "_",energy_exports, "_ratio_new.png"),  width = 20, height = 10, res = 600, units = "cm") # 20, 10
        }else if(difference == TRUE & ratio == FALSE) {
          png(file = paste0(plots_path, map, "-", id,"_", climate, "_", year[1], "_", region, file_label, "_",energy_exports, "_diff_new.png"),  width = 20, height = 10, res = 600, units = "cm") # 20, 10
          }else if(ratio == FALSE & difference == FALSE){
            png(file = paste0(plots_path, map, "-", id,"_", climate, "_", year[1], "_", region, file_label, "_",energy_exports, "_circles.png"),  width = 20, height = 10, res = 600, units = "cm") # 20, 10
              }
    }
  ...
  if ((!grepl("mammals", file_label, fixed = TRUE) && !grepl("birds", file_label, fixed = TRUE) && !grepl("plants", file_label, fixed = TRUE)) && ratio == FALSE && difference == FALSE) {
    figure <- figure +
      geom_point(data = df1, aes(x = longitude, y = latitude, size = totalPop), colour= "#00a3e8", alpha = 0.5)                 
