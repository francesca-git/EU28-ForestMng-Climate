#############################################################################################################################################################################
                                                                  ...
                                                                  # GLOBAL LAND USE IMPACTS AND GLOBAL AREAS - STACKED TIME SERIES #
...
plot.global.time.series <- function(csv_path, case_subcase, plots_path) {
  ...
  # load and prepare the the data
    ...
    data <- read.csv(paste0(csv_path, "global_time-series", case_subcase, ".csv"), header = TRUE)
  ...
  # plot the areas (reload the data and adpt the palette)
    ...
    data <- read.csv(paste0(csv_path_areas, "global_time-series_areas.csv"), header = TRUE)
    ...
    figure2 <- 
      ggplot(data, aes(x=Year, y=Area, fill = Category)) +
      geom_area(alpha=0.85, size= 0.2, color = "gray85") +
  ...
  # put the figures together
    ...
    ggsave(paste0(plots_path, "Global_time-series_", palette_name, case_subcase, ".pdf"), width = 21, height = 20, units = "cm")
