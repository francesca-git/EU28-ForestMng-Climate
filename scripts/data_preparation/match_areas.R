  ...
  #################################################################################                        
    ...
    for (i in 1:ntstep) {
     ...
     # Create a single dataframe which combines all the input data
      ...
      if(not_rel_wet == FALSE) {areas_in <- areas_in %>% select(-WetLnd, -NotRel)}
            ...
            # test =====
              # Check CrpLnd, GrsLnd and NatLnd in EU ====
                ...
                test <- areas_in %>% filter(Scenario == scenario_test, Ecoregion == ecoregion_test) 
                ...
                if((test$CrpLnd_EU != test_check$CrpLnd_EU - test_check$EP_CrpLnd_EU) | (test$GrsLnd_EU != test_check$GrsLnd_EU - test_check$EP_GrsLnd_EU) 
                  ...
                  stop("ERROR in section 2 - energy plantations or forests in EU")
                }
              ...
              # Check CrpLnd, GrsLnd and NatLnd outside EU =====
                ...
                if((test$CrpLnd_RoW != test_check$CrpLnd_RoW - test_check$EP_CrpLnd_im) | (test$GrsLnd_RoW != test_check$GrsLnd_RoW - test_check$EP_GrsLnd_im) | (test$NatLnd_RoW != test_check$NatLnd_RoW - test_check$EP_NatLnd_im)) {
                  stop("ERROR in section 2 - energy plantation import")
                }
        ...
        # Grasslands in GLOBIOM areas do not include pastures and rangelands, which are accounted in NatLnd. Therefore, part of NatLnd is allocated to GrsLnd according to FAO data ====
          ...
          areas_test <- areas_in
          areas_in[areas_in$NatLnd_EU > 0, "GrsLnd_EU"] <- areas_in[areas_in$NatLnd_EU > 0, "GrsLnd_EU"] + areas_in[areas_in$NatLnd_EU > 0, "NatLnd_EU"]*0.51
          areas_in[areas_in$NatLnd_EU > 0, "NatLnd_EU"] <- areas_in[areas_in$NatLnd_EU > 0, "NatLnd_EU"] - areas_in[areas_in$NatLnd_EU > 0, "NatLnd_EU"]*0.51
          areas_in[areas_in$NatLnd_RoW > 0, "GrsLnd_RoW"] <- areas_in[areas_in$NatLnd_RoW > 0, "GrsLnd_RoW"] + areas_in[areas_in$NatLnd_RoW > 0, "NatLnd_RoW"]*0.51
          areas_in[areas_in$NatLnd_RoW > 0, "NatLnd_RoW"] <- areas_in[areas_in$NatLnd_RoW > 0, "NatLnd_RoW"] - areas_in[areas_in$NatLnd_RoW > 0, "NatLnd_RoW"]*0.51
          
          areas.in[[toString(tstep[i])]] <- areas_in
      ...
      #3) Make all necessary allocations
          ...
          # Main allocation processes ====== 
            ...
            areas_in_disaggr <- areas.fin[[toString(tstep[i])]] %>%                                                                                 # subtract areas exported outside EU from EU forested areas
            ...
            # test ====
              ...
              if(not_rel_wet == FALSE) {
                ...
                test3a = sum(test3)
                if((test3a-test2a) > 1) {stop("ERROR - allocation of exceeding MngFor")}
                if((all.equal(test2[1:(length(test2)-3)], test3[1:(length(test3)-8)], tolerance = 1e-03) != TRUE) 
                   | (all.equal(test2[(length(test2)-1):length(test2)], test3[(length(test3)-7):(length(test3)-6)], tolerance = 1e-03) != TRUE)) {stop("ERROR - allocation of exceeding Mngfor, other land uses")}
                if(abs((test2["MngFor"] - sum(test3[(length(test3)-5):length(test3)]))) > 1) {stop("ERROR - allocation of exceeding Mngfor")}
              }
              
              if(not_rel_wet == TRUE) {
                test3 = colSums(areas_for_alloc[3:length(areas_for_alloc)])
                test3a = sum(test3)
                if(abs(test3a-test2a) > 1) {stop("ERROR - allocation of exceeding MngFor")}
              ...
              # Check separation of Urban areas ====
                ...
                test7 = areas_post_alloc
                  if ((sum(test6$Urban) - sum(test7$Urban_EU + test7$Urban_RoW)) > 1e-6) {stop("ERROR - separation of Urban areas")}
          ...
          # Allocation of negative values in forest use classes to the positive areas in the same forest category in the ecoregions belonging to the EU region =========
            ...
            areas_fin_EU <- areas_in_disaggr %>%                                 
                                      inner_join(EU_ecoregions)                                                 # extract the rows belonging to the EU region
            
            areas_fin_RoW <- setdiff(areas_in_disaggr, areas_fin_EU)                            # put the remaning rows in another df, such that after the allocation they can be binded again
            
            areas_fin_EU = Neg_allocation(areas_fin_EU, Globiom_eco_EU, "Selection_EU")         # allocate negative values in Selection (the allocation is done such as all the ecoregions belonging to EU do not have any share outside EU -> the Globiom share is set to 1)
            areas_fin_EU = Neg_allocation(areas_fin_EU, Globiom_eco_EU, "Clear_cut_EU")         # allocate negative values in Clear cut (the allocation is done such as all the ecoregions belonging to EU do not have any share outside EU -> the Globiom share is set to 1)
            areas_fin_EU = Neg_allocation(areas_fin_EU, Globiom_eco_EU, "Timber_plant_EU")      # allocate negative values in Timber plantations (the allocation is done such as all the ecoregions belonging to EU do not have any share outside EU -> the Globiom share is set to 1)
  
            areas_fin_disaggr = areas_fin_RoW %>%                # the EU and RoW ecoregions are binded together again
                                  bind_rows(areas_fin_EU) %>%
                                    arrange(Scenario, Ecoregion)
            
            rm(areas_fin_EU, areas_fin_RoW)
            
              # test ====
                test = colSums(areas_fin_disaggr[3:length(areas_fin_disaggr)])
                test_sum = sum(test)
                
                # Check allocation of negative forest use ====
                test_check = colSums(areas_in_disaggr[3:length(areas_in_disaggr)])
                test_check_sum = sum(test_check)
                if(round(test_sum - test_check_sum, 3) != 0) {stop("ERROR - allocation of negative values of forest use")}
                if(all.equal(test, test_check) != TRUE) {stop("ERROR - allocation of negative values of forest use")}
      ...
      #4) Arrange the columns to make them suitable for the model 
          ...
          # Disaggregated land use classes ======
            ...
            if(not_rel_wet == FALSE) {
              ...
              areas_fin_disaggr <- areas_fin_disaggr %>%
                transmute(Scenario = Scenario, Ecoregion = Ecoregion, A_org = rowSums(.[3:length(areas_fin_disaggr)]), A_new = NatLnd_EU + NatLnd_RoW + PriFor, 
              ...
              areas_fin_disaggr$Ecoregion <- as.factor(areas_fin_disaggr$Ecoregion) 
              areas.fin.disaggr[[toString(tstep[i])]] <- areas_fin_disaggr
          
            # Grouped land use classes ======
              
              areas_aggr = areas_fin_disaggr %>%
                                  transmute(Scenario = Scenario, Ecoregion = Ecoregion, A_org = A_org, A_new = A_new, Annual = Annual_EU + Annual_RoW, 
                                            ...
                                            For_SlashAndBurn = 0,
                                            For_SelectionSystem = For_SelectionSystem_EU + For_SelectionSystem_im + For_SelectionSystem_ex, 
            ...
            # Fraction of land use classes ======
                ...
                fr_areas <- areas_fin_disaggr %>%
                               full_join(areas_aggr) %>%
                transmute(Scenario = Scenario, Ecoregion = Ecoregion, 
                          ...
                          fr_For_SelectionSystem_EU = For_SelectionSystem_EU/For_SelectionSystem,
                          ...
                          fr_ForOther_Intensive_EU = ForOther_Intensive_EU/ForOther_Intensive, fr_ForOther_Intensive_RoW = ForOther_Intensive_RoW/ForOther_Intensive) #27 # we consider a single For_Regrowth, because Regrowth in EU is 0
                  
              fr_areas$Ecoregion <- as.factor(fr_areas$Ecoregion) 
              fr_areas[is.na(fr_areas)] <- 0
    
            ## test ========
              test_row <- sample(1:nrow(fr_areas),1)
              test <- fr_areas[test_row,]
              test_check <- areas_aggr[test_row,]
              ...
              if((test_check$Annual > 0 & abs(1 - (test$fr_Annual_EU + test$fr_Annual_RoW)) >  1e-15) | 
                 ...
                 (test_check$For_SelectionSystem > 0 & abs(1 - (test$fr_For_SelectionSystem_EU + test$fr_For_SelectionSystem_im + test$fr_For_SelectionSystem_ex)) > 1e-15) |
              ...
              {stop("ERROR - calculation of fractions")}
        ...
        # Not relevant lands and wetlands (for plotting) ======  
            ...
            if(not_rel_wet == TRUE) {
              areas_fin_disaggr_notReWet <- areas_fin_disaggr %>%
              transmute(Scenario = Scenario, Ecoregion = Ecoregion, A_org = rowSums(.[3:length(areas_fin_disaggr)]), A_new = NatLnd_EU + NatLnd_RoW + PriFor, 
                        Annual_EU = CrpLnd_EU, Permanent_EU = OagLnd_EU, Pasture_EU = GrsLnd_EU, Urban_EU = Urban_EU, 
                        Annual_RoW = CrpLnd_RoW, Permanent_RoW = OagLnd_RoW, Pasture_RoW = GrsLnd_RoW, Urban_RoW = Urban_RoW,
                        For_ClearCut_EU = Clear_cut_EU, For_ClearCut_im = Clear_cut_im, For_ClearCut_ex = Clear_cut_EU_ex, For_Retention_EU = Retention_EU, 
                        ...
                        Wetlands = WetLnd, NotRelevantLands = NotRel) #29 # we consider a single For_Regrowth, because Regrowth in EU is 0
