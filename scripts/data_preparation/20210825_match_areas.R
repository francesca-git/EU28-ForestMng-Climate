...
# Task: Take as input the areas tided up in tidy_areas.R and match the multiple datasets containit the data on land use
#       The output of this script are .csv files containing the areas per land use type per year (each year is a separate file).
...
match.areas <- function(timber, not_rel_wet) {
  ...
  if (timber == FALSE) { load("./scripts_preparation/areas-to-match.RData") # all areas corresponding to timber plantations are 0 Mha
  } else if(timber == TRUE) {load("./scripts_preparation/areas-to-match_timber.RData") } # Here we consider that part of the areas covered by clear cut in EU are allocated to Timber plantations.
