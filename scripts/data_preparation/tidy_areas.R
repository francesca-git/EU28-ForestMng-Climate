...
#       The output of this script is a .Rdata file meant to be used as input in match_areas.R.
#       There are two options of opearting the function: including or exluding timber plantations as replacement for clear-cut in some EU areas.
...
# working directory set with the script set-wd.R available in the main folder
  ...
  tidy.areas <- function(timber, areas_base_path) {
    ...
    if (read_the_data == TRUE) {
      ...
      forest_intensity_rcp <- read.csv(paste0(areas_base_path, "Forest_Intensity_rcp_Mha.csv"), header = TRUE) # Areas of Regrowth, Intensive forest management or Extensive forest management in the GLOBIOM regions for the more ambitious climatic mitigation scenario (RCP2.6), Mha 
      Globiom_eco_org <- read.csv(paste0(areas_base_path, "GLOBIOM_Ecoregion.csv"), header = TRUE) # mapping between GLOBIOM and the ecoregions, with the share of each ecoregion in each GLOBIOM region
      urban <- read.csv(paste0(areas_base_path, "Chaudhary_2015_Areas.csv"), header = TRUE,  # Urban areas
