...
source("./scripts/model/distributions.R")
  ...
  load.data <- function(ecodata_path) {
    
      #Ecoregions
      Ecoregions_tot = read.csv(paste0(ecodata_path, "Ecoregions_description.csv"), header = TRUE) # Ecoregions specifications from Chaudhary 2015
      ...
      CF_local_landuse = read.csv(paste0(ecodata_path, "CF_local.csv"), header = TRUE) # Local CF from SI of Chaudhary 2015, blu = broad land use (annual, permanent, pasture, urban)

      CF_local_forestmng <- read.csv(paste0(ecodata_path, "CF_local_forest-use.csv"), header = TRUE, col.names = c("Taxa_Used", "Land_use_type", "Ratio", "Local_CF")) # Local CF and response ratio from SI of Chaudhary 2016
      
      z_rawvalues <- read.csv(paste0(ecodata_path, "z_input-values.csv"), header = TRUE)
  ...
  prepare.zvalues <- function(uncertainties, ecodata_path) {  # uncertainties can be FALSE (no simulations are generated for the z values) 
    ...
    if (uncertainties == FALSE) {simulations = 2} else if (uncertainties != FALSE) {simulations = n}
      ...
      # load the data on the ecoregions
        data_loaded <- load.data(ecodata_path)
      ...
      # from Supplementary materials (Table S2) of De Baan (2013), Use in Life Cycle Assessment: Global Characterization Factors Based on Regional and Global Potential Species Extinction
        ...
        # z for island, forest and nonforest
        z = c(0.2825, 0.3290, 0.219) # these values are the median of the z values from Drakare et al. (2005) for mammals, birds and plants available in input_data/ecoregions_data/z_analysis/z_input-values.csv
      ...
      # assign the corresponding simulation result to each ecoregion according to the habitat type (islands, forests, not forests)
        ...
        if(uncertainties == TRUE) {save(zvalues, file = paste0(ecodata_path, "rr_z/zvalues_mc.Rdata"))
          }else if(uncertainties == FALSE) {save(zvalues, file = paste0(ecodata_path, "rr_z/zvalues_static.Rdata"))}
  ...
  prepare.Sorg.VS.weighting <- function(vulnerability, ecodata_path) { # Vulnerability can be TRUE (Sorg is multiplied by the vulnerability scores and the weighting 
  ...
  # output: (array)
  prepare.RR <- function(uncertainties, n, ecodata_path) { 
  ...
  calculate.RR <- function(uncertainties, distribution, n, cutoff, ecodata_path) {
  ...
  # uncertainties -> whether to calculate them or not (TRUE or FALSE)
  # distribution = type of distribution for the Monte Carlo simulations. Options: "Weibull" or "lognormal". If missing, default = "Weibull". 
  # n = number of replicates in the bootstrapping process. If missing, default = 1000 when uncertainties = TRUE. If uncertainties = FALSE, n = 2. 
  # cutoff <- whether to cut off the rr above 1 and covert them to 1 or not (TRUE or FALSE). Default = TRUE.
    ...
    if (uncertainties == FALSE) {n = 2}
    if (uncertainties == TRUE & (missing(n))) {n = 1000}
    simulations = n
    
    rr <- prepare.RR(uncertainties, n, ecodata_path)
    ...
    # indices_forest_mng = same as indices_land_use but for rr_forest_mng. 
      ...
      if (cutoff == TRUE){
      if(uncertainties == TRUE) {save(rr_ecoregion, file = paste0(ecodata_path, "rr_z/rr_ecoregion_mc.Rdata"))
      } else if(uncertainties == FALSE) {save(rr_ecoregion, file = paste0(ecodata_path, "/rr_z/rr_ecoregion_static.Rdata"))}
      } else if (cutoff == FALSE){
        if(uncertainties == TRUE) {save(rr_ecoregion, file = paste0(ecodata_path, "rr_z/rr_ecoregion_mc_nocutoff.Rdata"))
        } else if(uncertainties == FALSE) {save(rr_ecoregion, file = paste0(ecodata_path, "rr_z/rr_ecoregion_static_nocutoff.Rdata"))}
      }
