...
############################# DEFINITION OF THE SCENARIOS/CONDITIONS UNDER WHICH THE CALCULATION IS PERFORMED ############################# 
  ...
  # Calculation of the impacts
    ...
    Timber = FALSE            # TRUE (part of EU clear cut areas have been allocated to Timber plantations). !! This option is valid only if approach == "MG"
  ...
  # Calculation of the response ratio
    ...
    vulnerability = TRUE      # TRUE (global impacts are quantified) or FALSE
    BS = FALSE                # If CI = TRUE and BS = TRUE, confidence intervals are quantified with bootstrapping. If CI = TRUE and BS = FALSE, confidence intervals are quantified with MonteCarlo simulation. 
                              # if CI = FALSE, no confidence intervals are quantified so it does not matter which value is assigned to BS.

############################# INITIAL SETTINGS ############################# 
  
     necoregions = 804                      
     Ecoregions = read.csv("./ecoregions_data/Ecoregions_description.csv", header = TRUE) # Ecoregions specifications from Chaudhary et al. (2015)
     ...
     nscenarios = 24 # number of years (11) x number of forest management scenarios (6) x numbers of climate mitigation scenarios (2)
...
############################# CALCULATION OR LOADING OF THE INPUT PARAMETERS ############################# 
      ...
      if (CI == FALSE) { uncertainties = FALSE            # no confidence intervals
                         n = 2
      ...
      # LOADING OF THE RESPONSE RATIOS (all .Rdata which are loaded here below contain a 4-dimension array called rr_ecoregion. 1dim: ecoregions;
      ...
      Sorg_VS = prepare.Sorg.VS.weighting(vulnerability = vulnerability)[["Sorg_VS"]]      # dataframe containing the original number of species per ecoregion
    ...
    for (tstep in 1:ntsteps) {
      ...
      ############ CALCULATION OF SPECIES LOSS ############
        ...
        ################### COMPUTE THE TOTAL IMPACTS AND THE IMPACTS PER TAXONOMIC GROUP ################### 
          ...
          if (BS == FALSE) {      
          Slost <- calculate.slost(Total_RemainingNatural_areas = Areas_org_new, Land_use_areas = Areas_lu, param = parameters, CI = FALSE, BS) # function in calculate_slost.R
