...
#load("./ratio.RData")
source("./CF_functions.R")   # TO CHECK WITH THE ONE ON THE SERVER
...
fromCFtoRR = function(data)  {
  ...
  data <- lapply(data, function(x) sapply(x, function(x) if (x == 0) {x = NA} else {x = x}))
  data <- lapply(data, function(x) x[!is.na(x)])
  
  return(data)
}

# ! the following function can be used only for testing purposes (z values = unweighted average of z values)
fromCFtoh = function(data)  {
  data <- lapply(data, function(x) (1-x)^0.271) #from CF to ratio
  data <- lapply(data, function(x) sapply(x, function(x) if (x == 0) {x = NA} else {x = x}))
  data <- lapply(data, function(x) x[!is.na(x)])
...
plot_fromgroups <- function(input_data, test) {
  ...
  for (i in 1:length(input_data)) {
    ...
    if(length(data) < 5) {
      ...
      if (test == "pvalue") {
        ...
        plot_distr_AIC(data, title)
      } else {
      stop("ERROR - Indicate the test to be performed. Choice: 'pvalue' or 'AIC'")
      }
    }
  }
...
aic.rl <- function(min, aic) {
  result = exp((min-aic)/2)
  return(result)
}

plot_distr_AIC = function(data, title){
  # data <- ratio_morethan10[[8]]
  # title = names(ratio_morethan10[8])
  ...
  aicmin <- min(aicw, aicg, aicln)
  aicw_rl <- aic.rl(aicmin, aicw)
  aicg_rl <- aic.rl(aicmin, aicg) 
  aicln_rl <- aic.rl(aicmin, aicln) 
  
  
  print(paste0("AIC RL lnorm: ", round(aicln_rl, digits = 3)))
  print(paste0("AIC RL weibull: ", round(aicw_rl, digits = 3)))
  print(paste0("AIC RL gamma: ", round(aicg_rl, digits = 3)))
  
  plot.legend <- c(paste0("Weibull - ", round(aicw_rl, digits = 3)), 
                   paste0("lognormal - ", round(aicln_rl, digits = 3)), 
                   paste0("gamma - ", round(aicg_rl, digits = 3)))
  
  # denscomp(list(fw, fln, fg), addlegend = F, main = paste0(title, " - dp: ", length(data)), fitlty = c(1,2,5), fitlwd = c(1.5,1.5,1.5))
  # legend("topright", legend = plot.legend, cex = 0.8, col = c("red","green", "blue"), lty=c(1,2,5), lwd = c(1.5,1.5,1.5), box.lty=0, 
  #         title = "AIC relative likelihood")
  
  denscomp(list(fw, fln, fg), addlegend = TRUE, legendtext = plot.legend, xlegend = "topleft", xlab = "response ratio", cex = 0.8, 
           main = paste0(title, " - dp: ", length(data)), fitlty = c(1,2,5), fitlwd = c(1.5,1.5,1.5))
  # legend("topright", legend = plot.legend, cex = 0.8, col = c("red","green", "blue"), lty=c(1,2,5), lwd = c(1.5,1.5,1.5), box.lty=0, 
  #         title = "AIC relative likelihood")
...
ratio_forest = fromCFtoh(CF_local_forest)
...
ratio_forestuse = fromCFtoh(CF_local_forestuse)
...
ratio_morethan10 <- list_subsetting(ratio_global, 10)
...
data <- ratio_forest_birds[c(3,4,9)]

pdf(file = paste0("./RR_distributions/distr_RR_", name, ".pdf"), width = 10, height = 10)

par(mfrow = c(3, 2))

plot_fromgroups(data, test)

dev.off()

# test distribution on h or on RR

library(truncdist)

set.seed(1)
#data_fromh <- ratio_forest_plants
fw <- fitdist(data_fromh$SelectionSystem_Plants_Global, "weibull")
test <- rtrunc(1000, spec = "weibull", a = 0, b = 1, shape = fw$estimate[[1]], scale = fw$estimate[[2]])
hist(test)
test_sort <- sort(test)
test_sort[25]
test_sort[975]
...
set.seed(1)
#data_fromRR <- ratio_forest_plants
fw <- fitdist(data_fromRR$SelectionSystem_Plants_Global, "weibull")
test <- rtrunc(1000, spec = "weibull", a = 0, b = 1, shape = fw$estimate[[1]], scale = fw$estimate[[2]])
hist(test)
test <- test^0.271
test_sort <- sort(test)
...
# without cut-off, no big difference in the CI using h or RR to produce the distributions
# distribution on h 
# 0.7044798
# 1.247001
# distribution on RR
# 0.7257476
# 1.243816
# also with cut-off, no big difference
# distribution on h 
# 0.6733813
# 0.9966443
# distribution on RR
