...
calculate.impacts.pervolume.oneyear <- function(csv_path, year, file_label) {
  ...
  impacts <- read.csv(paste0(csv_path, "EUFootprint_", year, file_label, "_EP_disaggr.csv"), header = TRUE) %>%
  ...
  df <- inner_join(biomass_new, impacts_new, by = c("Pathway", "Group", "Scenario"))
  ...
  write.csv(df, paste0(csv_path, "PDF_Mm3_", year, file_label, ".csv"), row.names = FALSE)
