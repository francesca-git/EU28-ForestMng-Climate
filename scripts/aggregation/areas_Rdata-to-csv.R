...
# Author: Francesca Rosa
  ...
  create.csv.EU.areas <- function(file_rdata_path_areas, aggr_plot_path_areas, label_timber, year) {
    ...
    # load the data
      ...
      # focus on EU
        ...
        ################## EU #######################
          ...
          write.csv(results_oneyear, paste0(aggr_plot_path_areas, "areas_EUlanduse_", year, "_", label_timber, ".csv"), row.names = FALSE)
        ...
        ################## EU FOOTPRINT #######################
          ...
          # group the land use classes in: EU28_Energy_crops, EU_Forest_net, Import_Energy_plantations, Import_Forest
            ...
            write.csv(results_oneyear, paste0(aggr_plot_path_areas, "areas_EUFootprint_", year, "_" , label_timber, ".csv"), row.names = FALSE)
            # this csv has the following columns: 
            # Group (climatic and forest use scenario), Scenario (forest management scenario), Category (land use category), Values (areas)
        ...
        ################## EU INTERNAL #######################
          ...
          ######## SUMS OF GROUPED LAND USE CLASSES IN THE EU (LAND USE AREA USED ONLY FOR DOMESTIC CONSUMPTION, SO EXPORTS ARE EXCLUDED) ########
          # group the land use classes in: Clear_cut, Retention, Selection, Timber, Other_management
            ...
            write.csv(results_oneyear, paste0(aggr_plot_path_areas, "areas_EUForest_", year, "_" , label_timber, ".csv"), row.names = FALSE)
            write.csv(results_points, paste0(aggr_plot_path_areas, "areas_EUForest_", year, "_" , label_timber, "_points.csv"), row.names = FALSE)
...
# Author: Francesca Rosa
  ...
  create.csv.global.areas <- function(file_rdata_path_areas, aggr_plot_path_areas, label_timber, year) {
  ...
  # Task: Produce a .csv that contain the areas per land use group or category 
    ...
    # change the format (before: one column per each land use class; after the change: one single column with al the values and one with the corresponding Category)
      ...
      write.csv(results_oneyear, paste0(aggr_plot_path_areas, "areas_global_", year, ".csv"), row.names = FALSE)
      # this csv has the following columns: 
      # Group (climatic and forest use scenario), Scenario (forest management scenario), Category (land use category), Values (areas)
                    ...
                    test_check = results[[year]][nrow(results_temp),]
                        ...
                        if (abs(test$Afforestation - (test_check$Afforested_EU + test_check$Afforested_RoW)) > 1e-16  |
                              abs(test$Regrowth - test_check$Regrowth) > 1e-16 |
                              abs(test$Annual_crops - (test_check$Annual_EU + test_check$Annual_RoW)) > 1e-16  |
                              abs(test$Energy_crops_and_plantations - (test_check$EP_EU + test_check$EP_conv_EU + test_check$EP_RoW + test_check$EP_conv_im)) > 1e-16  |
  ...
  ##################################################### 2b) TIME SERIES OF IMPACTS ##################################################### 
   ...
   # TIME SERIES 
    ...
    # Task: Produce one .csv that contains the areas (Mha) per scenario, per land use group and per year 
      ...
      for(i in 1:length(tstep)) {
        ...
        # test ====
                     ...
                     test_check = results[[i]] %>% filter(Forest_use == "MFM" & Group == "RCP2.6" & Management == "noAFM") 
                         ...
                         if(abs(test$Afforestation - (test_check$Afforested_EU + test_check$Afforested_RoW)) > 1e-11 |
                               abs(test$Regrowth - test_check$Regrowth) > 1e-11 |
                               abs(test$Annual_crops - (test_check$Annual_EU + test_check$Annual_RoW)) > 1e-11 |
      ...
      write.csv(df, paste0(aggr_plot_path_areas, "areas_global_time-series_", label_timber, ".csv"), row.names = FALSE)
      # this csv has the following columns: 
      # Group (climatic and forest use scenario), Scenario (forest management scenario), Category (land use category), Values (areas)
