...
# GLOBAL IMPACTS AND CONFIDENCE INTERVALS

# Task: Produce four .csv that contain the CI of species lost (PDF) 
        # 1) per scenario and summed over all land use categories for 2 given years selected here below 
        # 2) per scenario and per land use group for 2 given years selected here below (impacts of land use categories of the same group are summed, e.g. the impacts of all forest land use categories are summed together). 
...
################## 1) TOTAL SUMS #######################
  ...
  # prepare and save the file with the total sums for the given year 
      ...
      write.csv(sums_oneyear, paste0(save_csv_path, "global_", year, case, "_top_CI.csv"), row.names = FALSE)
...
################## 2) SUMS PER LAND USE GROUP #######################
    ...
    # change the format (before: one column per each land use class; after the change: one single column with al the values and one with the corresponding Category)
      ...
      write.csv(results_oneyear, paste0(save_csv_path, "global_disaggr_", year, case, "_CI.csv"), row.names = FALSE)
  ...
  test_check = results[[year]][nrow(results_temp),]
      ...
      if ((abs((test$Afforestation - (test_check$Afforested_EU_median + test_check$Afforested_RoW_median))) > 1e-16) |
          (abs((test$Regrowth - test_check$Regrowth_median)) > 1e-16) |
      (abs(test$Annual_crops - (test_check$Annual_EU_median + test_check$Annual_RoW_median)) > 1e-16) |
      (abs(test$Energy_crops_and_plantations - (test_check$EP_EU_median + test_check$EP_conv_EU_median + test_check$EP_RoW_median + test_check$EP_conv_im_median)) > 1e-16) |
      (abs(test$Forest - (test_check$For_ClearCut_EU_median +  test_check$For_ClearCut_im_median + test_check$For_ClearCut_ex_median +
        ...
        test_check$ForOther_Extensive_RoW_median + test_check$ForOther_Intensive_EU_median + test_check$ForOther_Intensive_RoW_median)) > 1e-16) |
      (abs(test$Pasture - (test_check$Pasture_EU_median + test_check$Pasture_RoW_median)) > 1e-16) |
      (abs(test$Permanent_crops - (test_check$Permanent_EU_median + test_check$Permanent_RoW_median)) > 1e-16)  |
      (abs(test$Urban - (test_check$Urban_EU_median + test_check$Urban_RoW_median)) > 1e-16 )) {stop("ERROR in the aggregation of land use at global level")}
