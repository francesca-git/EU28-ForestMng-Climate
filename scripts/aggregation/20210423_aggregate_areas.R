...
aggregate.areas <- function(folder_areas, rdata_path_areas) {

    select <- dplyr::select
    
    temp = list.files(folder_areas, pattern="*.csv", full.names = TRUE)   # save as list the paths of all .csv files in the selected folder
    ...
    save(results, paste0(rdata_path_areas, "/areas.RData")) # this data can then be read by the scripts used to aggregate the land uses
