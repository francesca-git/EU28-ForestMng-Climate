...
# General task: calculate the impacts (PDF) per unit of volume. This function has to be used after calculate.slost.GLOBIOM, as first results the results are re-scaled
#               at the resolution of GLOBIOM regions and then it is possible to quantify the impacts per unit of volume (the volumes are provided per GLOBIM region)
# Date: April 2021
...
calculate.impacts.pervolume <- function(csv_path, file_label, areas_base_path) {
    ...
    write.csv(df_ratio, paste0(csv_path, "PDF_Mm3_globiomregions", file_label, ".csv"), row.names = FALSE)
    ...
    write.csv(df_ratio_sum, paste0(csv_path, "PDF_Mm3", file_label, ".csv"), row.names = FALSE)
