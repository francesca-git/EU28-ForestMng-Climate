...
plot.EU.barplot <- function(data, data_top, palette_name) {
    ...
    if((round(max(data_top$PDFx100), digits = 3) - max(data_top$PDFx100)) < 0.005) 
      { ymax_value <- (round(max(data_top$PDFx100), digits = 2) + 0.01)
          } else { ymax_value <- round(max(data_top$PDFx100), digits = 2) }
    
    if (min(data$PDFx100) >= 0) {ymin_value = 0}else{      
    if((min(data$PDFx100) - round(min(data$PDFx100), digits = 3)) > -0.005) 
      { ymin_value <- (round(max(data$PDFx100), digits = 2) - 0.01)
    } else { ymin_value <- round(min(data$PDFx100), digits = 2) }}
