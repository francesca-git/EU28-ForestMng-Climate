...
#############################################################################################################################################################################
  ...
  create.csv.global <- function(CI, rdata_path, csv_path, case_subcase) {
  ...
  ################## 1) TOTAL SUMS #######################
    ...
    # prepare and save the file with the total sums for the given year 
        ...
        sums_oneyear = sums[[year]] %>% 
          ...
          if (CI == TRUE) { write.csv(sums_oneyear, paste0(csv_path, "global_", year, case_subcase, "_top_CI.csv"), row.names = FALSE) 
            } else if (CI == FALSE) { write.csv(sums_oneyear, paste0(csv_path, "global_", year, case_subcase, "_top.csv"), row.names = FALSE) }
  ...
  ################## 2) SUMS PER LAND USE GROUP #######################
      ...
      # change the format (before: one column per each land use class; after the change: one single column with al the values and one with the corresponding Category)
        ...
        write.csv(results_oneyear, paste0(csv_path, "global_", year, case_subcase, ".csv"), row.names = FALSE)
        # this csv has the following columns: 
  ...
  ################## 3) NO GROUPING, DISAGGREGATED RESULTS #######################
          ...
          write.csv(results_temp, paste0(csv_path, "global_tot-disaggr_", year, case_subcase, ".csv"), row.names = FALSE)
  ...
  ##################################################### TIME SERIES OF IMPACTS ##################################################### 
    ...
    # Task: Produce one .csv that contains the impacts in terms of species lost (PDF) 
      ...
      write.csv(df, paste0(csv_path, "global_time-series", case_subcase, ".csv"), row.names = FALSE)
